...
//        partition_v3(rule,subset_4,num_subset_4,number_rule,field_length_ruleset,threshold_value); //四个规则集划分sa_da sa da big
        ...
        printf("CutTSS\n");
        CutTSS ct_sa(0,threshold,subset_3[0]);
        CutTSS ct_da(1,threshold,subset_3[1]);
        PriorityTupleSpaceSearch ptss;
        //TSS ptss;

        start = std::chrono::steady_clock::now();
        //construct classifier
        if(num_subset_3[0] > 0) ct_sa.ConstructClassifier(subset_3[0]);
        if(num_subset_3[1] > 0) ct_da.ConstructClassifier(subset_3[1]);
        ...
        if(num_subset_3[2] > 0) ptss2.ConstructClassifier(subset_3[2]);
        end = std::chrono::steady_clock::now();
        elapsed_milliseconds = end - start;
        ...
        if(fpt != NULL){
            ...
            for (int t = 0; t < trials; t++) {
                ...
                for (auto const &p : packets) {
                    match_pri = -1;
                    if(num_subset_3[0] > 0) match_pri = max(match_pri, ct_sa.ClassifyAPacket(p));
                    if(num_subset_3[1] > 0 && match_pri < max_pri[1]) match_pri = max(match_pri, ct_da.ClassifyAPacket(p));
...
//            printf("\t%d packets are classified, %d of them are misclassified\n", number_pkt, match_miss);
            ...
            int total_query = ct_sa.TablesQueried()+ct_da.TablesQueried()+ptss.TablesQueried();
            ...
            for (auto const &p : packets) {
                ...
                if(num_subset_3[2] > 0 && match_pri < max_pri[2]) match_pri = max(match_pri, ptss2.ClassifyAPacket(p));
                //results.push_back(pri_id[match_pri]);
            }
            end = std::chrono::steady_clock::now();
            elapsed_seconds = end - start;
...
//            printf("\t%d packets are classified, %d of them are misclassified\n", number_pkt, match_miss);
            ...
            printf("\tAverage classification time: %f us\n", sum_time3.count()*1000000 / (trials*packets.size()));
            printf("\tThroughput: %f Mpps\n", 1 / (sum_time3.count()*1000000 / (trials*packets.size())));
            total_query = bt_sa.TablesQueried()+bt_da.TablesQueried()+ptss2.TablesQueried();
            printf("\tTotal memory access: %d\n", total_query);
            printf("\tAverage memory access: %f\n", 1.0 * total_query / (trials * packets.size()));
...
//                {
//                    if(smask>=prefixLength) ct_sa.InsertRule(rule[ra]);
//                    else if(dmask>=prefixLength) ct_da.InsertRule(rule[ra]);
//                    else ptss.InsertRule(rule[ra]);
//                    insert_num++;
//                } else{//1:delete
//                    if(smask>=prefixLength) ct_sa.DeleteRule(rule[ra]);
//                    else if(dmask>=prefixLength) ct_da.DeleteRule(rule[ra]);

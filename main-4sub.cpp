 ...
 * =====================================================================================
 */
void partition_v2(vector<Rule> rule,vector<Rule> subset[3],int num_subset[3],int number_rule,field_length *field_length_ruleset,int threshold_value[2])
...
//        partition_v2(rule,subset_3,num_subset_3,number_rule,field_length_ruleset,threshold_value); //三个规则集划分sa da big
        ...
        BitTSS bt_sa_da(0,threshold,subset_4[0]);
        BitTSS bt_sa(0,threshold,subset_4[1]);
        BitTSS bt_da(1,threshold,subset_4[2]);
        PriorityTupleSpaceSearch ptss2;

        start = std::chrono::steady_clock::now();
        //construct classifier
        if(num_subset_4[0] > 0) bt_sa_da.ConstructClassifier(subset_4[0]);
        if(num_subset_4[1] > 0) bt_sa.ConstructClassifier(subset_4[1]);
        if(num_subset_4[2] > 0) bt_da.ConstructClassifier(subset_4[2]);
        if(num_subset_4[3] > 0) ptss2.ConstructClassifier(subset_4[3]);
        end = std::chrono::steady_clock::now();
        elapsed_milliseconds = end - start;
        ...
        if(fpt != NULL){
            ...
            for (int t = 0; t < trials; t++) {
                ...
                for (auto const &p : packets) {
                    match_pri = -1;
//                    if(num_subset_4[0] > 0) match_pri = max(match_pri, ct_sa_da.ClassifyAPacket(p));
//                    if(num_subset_4[1] > 0) match_pri = max(match_pri, ct_sa.ClassifyAPacket(p));
//                    if(num_subset_4[2] > 0) match_pri = max(match_pri, ct_da.ClassifyAPacket(p));
//                    if(num_subset_4[3] > 0) match_pri = max(match_pri, ptss.ClassifyAPacket(p));
                    if(num_subset_4[0] > 0) match_pri = max(match_pri, ct_sa_da.ClassifyAPacket(p));
                    if(num_subset_4[1] > 0 && match_pri < max_pri[1]) match_pri = max(match_pri, ct_sa.ClassifyAPacket(p));
                    if(num_subset_4[2] > 0 && match_pri < max_pri[2]) match_pri = max(match_pri, ct_da.ClassifyAPacket(p));
                    if(num_subset_4[3] > 0 && match_pri < max_pri[3]) match_pri = max(match_pri, ptss.ClassifyAPacket(p));
            ...
            printf("BitTSS\n");
            std::chrono::duration<double> sum_time3(0);
            match_pri = -1;
            results.clear();
            for (int t = 0; t < trials; t++) {
            start = std::chrono::steady_clock::now();
            for (auto const &p : packets) {
                match_pri = -1;
                if(num_subset_4[0] > 0) match_pri = max(match_pri, bt_sa_da.ClassifyAPacket(p));
                if(num_subset_4[1] > 0 && match_pri < max_pri[1]) match_pri = max(match_pri, bt_sa.ClassifyAPacket(p));
                if(num_subset_4[2] > 0 && match_pri < max_pri[2]) match_pri = max(match_pri, bt_da.ClassifyAPacket(p));
                if(num_subset_4[3] > 0 && match_pri < max_pri[3]) match_pri = max(match_pri, ptss2.ClassifyAPacket(p));
                results.push_back(pri_id[match_pri]);
            }
            end = std::chrono::steady_clock::now();
            elapsed_seconds = end - start;
            sum_time3 += elapsed_seconds;
            }
            match_miss = 0;
            for(int i = 0;i < number_pkt;i++){
                if(results[i] == -1) match_miss++;
            ...
            printf("\t%d packets are classified, %d of them are misclassified\n", number_pkt, match_miss);
            printf("\tTotal classification time: %f s\n", sum_time3.count() / trials);
            printf("\tAverage classification time: %f us\n", sum_time3.count()*1000000 / packets.size());
            total_query = bt_sa_da.TablesQueried()+bt_sa.TablesQueried()+bt_da.TablesQueried()+ptss2.TablesQueried();
            printf("\tTotal tables queried: %d\n", total_query);
            printf("\tAverage tables queried: %f\n", 1.0 * total_query / (trials * packets.size()));
            ...
            printf("\tAverage classification time: %f us\n", sum_time1.count()*1000000 / packets.size());
            printf("\tTotal tables queried: %d\n", ptss1.TablesQueried());
            ...
            printf("\tAverage classification time: %f us\n", sum_time2.count()*1000000 / packets.size());
            printf("\tTotal tables queried: %d\n", ps.TablesQueried());

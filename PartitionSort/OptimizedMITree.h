...
public:
	virtual int ClassifyAPacket(const Packet& packet,int *query) const = 0;
...
public:
   ...
   //		} 
	...
	int ClassifyAPacket(const Packet& one_packet, int *query)const {  //here
		return  RBExactQueryIterative(root, one_packet,  fieldOrder, query);

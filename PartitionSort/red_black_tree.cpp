...
int inline CompareQuery(const box& a, const Packet& q, int level, const std::vector<int>& fieldOrder) {
	...
	const int LOW = 0, HIGH = 1;
	if (a[HIGH] < q[fieldOrder[level]]) { //field order 规则子集域序
		return -1;
...
int RBExactQueryIterative(rb_red_blk_tree*  tree, const Packet& q, const std::vector<int>& fieldOrder, int *query) {
	 
	int level = 0;
	int compVal;
	while (true) { 
		//check if singleton 
		if (level == fieldOrder.size()) { // 已遍历完所有层 5
			return tree->GetMaxPriority(); //返回匹配的节点中最大优先级的规则
		} else if (tree->count == 1) { //该节点只有一个规则
			//  auto chain_boxes = tree->chain_boxes;
			for (size_t i = level; i < fieldOrder.size(); i++) {  //则判断各个域是否匹配
				if (q[fieldOrder[i]] < tree->chain_boxes[i - level][0]) return -1;
				if (q[fieldOrder[i]] > tree->chain_boxes[i - level][1]) return -1;
			}
		 
			return tree->GetMaxPriority();//如果匹配，则返回该规则的优先级
		}
